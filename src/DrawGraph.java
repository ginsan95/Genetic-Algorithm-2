...
public class DrawGraph {
	...
	{
		final int LOOP = 3;
		double[][] countGenArray = new double[LOOP][];
		...
		for(int i=0; i<functions.length; i++)
		{
			...
			{
				double[] minArray = new double[GENERATIONS];
				countGenArray[loop] = minArray;
				...
				minArray[0] = population.getMinValue();
				...
				//for(int count=2; population.getMinValue()>0.000000001; count++)
				{
					...
					CountDownLatch latch = new CountDownLatch(population.getSize()/2);
					...
					minArray[count-1] = population.getMinValue();
			...
			LineChart chart = new LineChart(
					functions[i] + " - MinValue vs Generations - ",
					functions[i] + " - MinValue vs Generations - 0 to " + GENERATIONS,
					countGenArray, 0, GENERATIONS);
			chart.pack( );
		    ...
		    LineChart chart2 = new LineChart(
		    		functions[i] + " - MinValue vs Generations - ",
		    		functions[i] + " - MinValue vs Generations - "+(GENERATIONS-200)+" to "+GENERATIONS,
					countGenArray, GENERATIONS-200, GENERATIONS);
			chart2.pack( );
...
{
	double[][] countGenArray;
	int start, end;
	public LineChart( String applicationTitle , String chartTitle , double[][] a, int s, int e)
	   {
	      ...
	      DecimalFormat pctFormat = new DecimalFormat("0.00000000000");
	   ...
	   private DefaultCategoryDataset createDataset( )
	   {
	      ...
	      for(int i=0; i<countGenArray.length; i++)
		  {
		      for(int j = start; j < end-1; j++){
		          dataset.addValue( countGenArray[i][j] , String.valueOf(i) , String.valueOf(j+1));
		      }
		  }
	      return dataset;
	   }
